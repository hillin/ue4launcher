...
namespace UE4Launcher.Launcher
{
	internal class LaunchProfileViewModel : NotificationObject, ITrayContextMenuItem
	{
		...
		public LaunchProfileViewModel(LaunchProfile profile, bool editMode)
		{
		...
		private void LoadAvailableCultures()
		{
			...
			this.AvailableCultures =
				allCultures.Union(allCultures.Select(c => c.Parent).Distinct())
						   .Where(c => !object.Equals(c, CultureInfo.InvariantCulture))
						   .Where(c => definedCultures.Count == 0 || definedCultures.Contains(c.Name.ToLowerInvariant()))
						   .Select(
							   c =>
		...
		public void Launch(IDebuggerInfo debugger)
		{
